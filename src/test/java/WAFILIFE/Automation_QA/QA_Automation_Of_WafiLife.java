...
public class QA_Automation_Of_WafiLife extends Setup_Browser {
	 ...
	 WebElement Name = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='billing_first_name']")));
        ...
        WebElement DistrictOption = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("/html[1]/body[1]/span[1]/span[1]/span[2]/ul[1]/li[13]")));
		try {
			if(DistrictOption.isDisplayed()) {
      	...
      	WebElement SubDistrictOption = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//option[@value='498'] ")));
        SubDistrictOption.click();
        
        WebElement add = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//textarea[@id='billing_address_1']")));
        try {
        	if(add.isDisplayed()) {
        		add.sendKeys("H:164/A Master Para (Koborsthan Jame Mashjid)");
        		Thread.sleep(1000);
        	}
        	}
        	catch(Exception e) {
        ...
        WebElement otherinfo = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//textarea[@id='order_comments']")));
        try {
        	if(otherinfo.isDisplayed()) {
        		otherinfo.sendKeys("N/A");
        		Thread.sleep(1000);
        	}
        	}
        catch (Exception e)
        ...
        WebElement ShippingCharge = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='shipping_method_0_flat_rate8']")));
        try {
        	if (ShippingCharge.isEnabled()) {
        ...
        WebElement paymentmethod = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='payment_method_nagad']")));
        try {
